...
def fill_holes(mesh):
    ...
    def hole_to_faces(hole, vertices, new_vertex):
        """
        ...
        if len(hole > 4):
            ...
            #print('New center', hole_center)
            center_idx = vertices.shape[0] + len(new_vertex) # Index of where the new vertex will be inserted
            new_faces = np.zeros((len(hole), 3))

            for i in range(len(hole)):
                next_vert = (i + 1) % len(hole)
                new_faces[i, :] = [hole[i], hole[next_vert], center_idx]

            return new_faces.astype(int), [hole_center]
    ...
    for hole in cycles:
        ...
        faces, vertex = hole_to_faces(hole=hole, vertices=mesh.vertices, new_vertex=new_vertex)
        if len(faces) == 0:
            continue
